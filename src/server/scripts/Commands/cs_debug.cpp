    ...
    public:
        ...
        static bool HandleDebugEnterVehicleCommand(ChatHandler* handler, char const* args)
        {
            ...
            else
            {
				...
				MoPCore::AllCreaturesOfEntryInRange check(handler->GetSession()->GetPlayer(), entry, 20.0f);
				MoPCore::CreatureSearcher<MoPCore::AllCreaturesOfEntryInRange> searcher(handler->GetSession()->GetPlayer(), passenger, check);
