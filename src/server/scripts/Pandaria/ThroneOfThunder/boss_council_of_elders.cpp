                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_POSSESS_NEXT:
                        {
                            ...
                            if (councilorToPossess)
                            {
                                ...
                                possessedCount++;
                                if (me->GetMap()->GetDifficulty() == RAID_STATUSFLAG_25MAN_HEROIC || me->GetMap()->GetDifficulty() == RAID_STATUSFLAG_10MAN_HEROIC && possessedCount % 2 == 0)
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CHECK_TREACH_GROUND:
			            {
                            if (!despawnable)
                            {
								if (Creature* Quicksand = me->FindNearestCreature(NPC_QUICKSAND, 4.0f * me->GetObjectSize(), true))
                                {
