...
#include "../scripts/Pandaria/ThroneOfThunder/throne_of_thunder.h"
...
public:
	...
	struct boss_ji_kunAI : public BossAI
	{
		...
		void JustReachedHome() override
		{
			...
			if (m_Instance)
				m_Instance->SetBossState(DATA_JI_KUN, EncounterState::FAIL);
		...
		void JustDied(Unit* p_Killer) override
		{
			if (m_Instance)
			{
				...
				m_Instance->SetBossState(DATA_JI_KUN, EncounterState::DONE);
			}
...
public:
	...
	struct creature_feed_pool_projectileAI : public ScriptedAI
	{
		...
		void UpdateAI(const uint32 p_Diff) override
		{
			...
			(events.ExecuteEvent()); //this line gave a error, idk if this will fix it.
			{
			}
		}
	};
