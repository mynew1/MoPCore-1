                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_BELLOWING_ROAR: // Phase PHASE_END
                        {
							...
							MoPCore::GameObjectInRangeCheck check(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 15);
							MoPCore::GameObjectLastSearcher<MoPCore::GameObjectInRangeCheck> searcher(me, Floor, check);
