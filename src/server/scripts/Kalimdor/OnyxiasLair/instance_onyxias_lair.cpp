    ...
    struct instance_onyxias_lair_InstanceMapScript : public InstanceScript
    {
        ...
        void FloorEruption(uint64 floorEruptedGUID)
        {
            if (GameObject* pFloorEruption = instance->GetGameObject(floorEruptedGUID))
            {
				...
				MoPCore::GameObjectInRangeCheck check(pFloorEruption->GetPositionX(), pFloorEruption->GetPositionY(), pFloorEruption->GetPositionZ(), 15);
				MoPCore::GameObjectListSearcher<MoPCore::GameObjectInRangeCheck> searcher(pFloorEruption, nearFloorList, check);
