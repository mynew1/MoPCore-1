                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DREADFLAME:
                        {
                            ...
                            if (t_pos.size() > max)
								MoPCore::Containers::RandomResizeList(t_pos, max);
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CHECK_CLOUDBURST:
                        {
							...
							MoPCore::UnitListSearcher<CloudburstCheck> searcher(me, targets, check);
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CHECK_TARGET:
                        {
							...
							MoPCore::AnyPlayerInObjectRangeCheck check(me, 3.0f, true);
							MoPCore::PlayerListSearcher<MoPCore::AnyPlayerInObjectRangeCheck> searcher(me, targets, check);
                            me->VisitNearbyObject(3.0f, searcher);
                            if (!targets.empty())
                            {
        ...
        class spell_ragnaros_firelands_wrath_of_ragnaros_aoe_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                if (!targets.empty())
                {
					...
					MoPCore::Containers::RandomResizeList(targets, max_size);
                }
            }
    ...
    public:
        ...
        class spell_ragnaros_firelands_magma_trap_aoe_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                {
                    targets.clear();
					targets.push_back(MoPCore::Containers::SelectRandomContainerElement(tempList));
                }
                else
