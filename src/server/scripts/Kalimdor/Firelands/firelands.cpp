...
static void AlysrazorTrashEvaded(Creature* creature)
{
	...
	MoPCore::CreatureWorker<TrashRespawnWorker> worker(creature, check);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SUMMON_SMOULDERING_HATCHLING:
                        {
							...
							MoPCore::CreatureListSearcher<MoltenEggCheck> searcher(me, eggs, check);
                            me->VisitNearbyGridObject(20.0f, searcher);
                            if (!eggs.empty())
                            {
								Creature* egg = MoPCore::Containers::SelectRandomContainerElement(eggs);
