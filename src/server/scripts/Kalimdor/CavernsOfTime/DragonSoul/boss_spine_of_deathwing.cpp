    ...
    public:
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_CORRUPTED_BLOOD:
                        {
                            ...
                            {
                                num = targets.size();
								Unit* pSpawner = MoPCore::Containers::SelectRandomContainerElement(targets);
        ...
        private:
            ...
            void DoRoll(uint8 side /* 1 - left, 2 - right*/)
            {
                ...
                MoPCore::PlayerListSearcher<AnyLivePlayerNoGmCheck> searcher(me, players, check);
            ...
            bool CheckPlayers()
            {
				...
				MoPCore::PlayerSearcher<AnyLivePlayerNoGmCheck> searcher(me, player, check);
                me->VisitNearbyWorldObject(200.0f, searcher);
        ...
        class spell_spine_of_deathwing_blood_corruption_AuraScript : public AuraScript
        {
            ...
            void HandleDispel(DispelInfo* dispelInfo)
            {
                ...
                if (Unit* owner = GetUnitOwner())
                {
					...
					MoPCore::PlayerListSearcher<PlayersCheck> searcher(owner, players, check);
                    owner->VisitNearbyObject(200.0f, searcher);
                    if (!players.empty())
						target = MoPCore::Containers::SelectRandomContainerElement(players);
                    else
                        target = owner;
