...
void hyjalAI::HideNearPos(float x, float y)
{
	...
	MoPCore::AllFriendlyCreaturesInGrid creature_check(me);
	MoPCore::CreatureListSearcher<MoPCore::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

	TypeContainerVisitor <MoPCore::CreatureListSearcher<MoPCore::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
	...
	MoPCore::WorldObjectWorker<MoPCore::RespawnDo> worker(me, u_do);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
			<MoPCore::CreatureListSearcher<MoPCore::AllFriendlyCreaturesInGrid>,
