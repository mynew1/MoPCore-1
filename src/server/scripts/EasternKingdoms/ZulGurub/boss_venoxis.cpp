        ...
        class spell_venoxis_toxic_link_aura_AuraScript : public AuraScript
        {
            ...
            void PeriodicTick(constAuraEffectPtr aurEff)
            {
                if (!GetCaster() || !GetTarget())
                 ...
                 UnitList targets;
				 MoPCore::AnyUnitHavingBuffInObjectRangeCheck u_check(GetCaster(), GetTarget(), 100, SPELL_TOXIC_LINK_AURA, true);
				 MoPCore::UnitListSearcher<MoPCore::AnyUnitHavingBuffInObjectRangeCheck> searcher(GetTarget(), targets, u_check);
