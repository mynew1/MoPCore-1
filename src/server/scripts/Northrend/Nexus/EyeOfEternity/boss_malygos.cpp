        ...
        class spell_malygos_arcane_storm_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                if (GetSpellInfo()->Id == SPELL_ARCANE_STORM_P_III)
                {
					...
					MoPCore::RandomResizeList(targets, check, (malygos->GetMap()->GetDifficulty() == MAN10_DIFFICULTY ? 4 : 10));
                }
                else
					MoPCore::RandomResizeList(targets, (malygos->GetMap()->GetDifficulty() == MAN10_DIFFICULTY ? 4 : 10));
            }
            ...
            void FilterMeleeHoverDiskPassangers(std::list<WorldObject*>& targets)
            {
                ...
                if (targets.size() > 1)
                    if (malygos && malygos->AI()->GetGUID(DATA_LAST_TARGET_BARRAGE_GUID))
						targets.remove_if(MoPCore::ObjectGUIDCheck(malygos->AI()->GetGUID(DATA_LAST_TARGET_BARRAGE_GUID)));
                ...
                if (!targets.empty())
					targets.remove_if(MoPCore::UnitAuraCheck(true, SPELL_ARCANE_BARRAGE_DAMAGE));
                ...
                {
                    if (targets.size() > 1)
						MoPCore::RandomResizeList(targets, 1);
    ...
    public:
        ...
        class spell_malygos_surge_of_power_warning_selector_25_SpellScript : public SpellScript
        {
            ...
            void SendThreeTargets(std::list<WorldObject*>& targets)
            {
				...
				MoPCore::RandomResizeList(selectedTargets, 3);
