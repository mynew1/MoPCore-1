...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
	MoPCore::NearestGameObjectCheck check(*pl);
	MoPCore::GameObjectLastSearcher<MoPCore::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
		...
		CellCoord p(MoPCore::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

		MoPCore::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
		MoPCore::GameObjectSearcher<MoPCore::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

		TypeContainerVisitor<MoPCore::GameObjectSearcher<MoPCore::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
